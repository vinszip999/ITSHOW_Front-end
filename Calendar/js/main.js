window.onload = function () {
  ...
  function makeElement(firstDate) {
    ...
    for (let i = 0; i < 6; i++) {
      ...
      for (let j = 0; j < 7; j++) {
        ...
        // firstDate.getMonth() => 현재 달의 일수가 몇일인지 반환해주고, 이 조건은 반환 된 값에 따라 출력해 준 후, 달력 출력 종료조건
  ...
  function delWork(e) {
    ...
    // DATA[currentDate]를 filter함수를 이용해 todo로 돌면서 todo의 아이디값과 현재 내가 누른 아이디값이 같지 않은 것을 배열에 담아 리턴해줘서 
